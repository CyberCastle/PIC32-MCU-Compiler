...
(cd xc32; ../generator/rg +v "$VERSION_ID (`echo $MCHP_RESOURCE | egrep -o \"[[:alnum:]]\"`)" -DMCHP_VERSION=${VERSION_ID} -DMCHP_RESOURCE=${MCHP_RESOURCE:-$CMD_RVERSION} xc32_devices.res xc32_device.info xc32_device.LanguageToolSuite deviceSupport.xml)
