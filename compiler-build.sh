...
if [ "x$LINUX32IMAGE" != "x" ]; then
    make DESTROOT="$WORKING_DIR/$LINUX32IMAGE/pic32-tools" install-headers 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
make DESTROOT="$WORKING_DIR/win32-image/pic32-tools" install-headers 2>&1 | tee -a "$LOGFILE" >>/dev/null
assert_success $? "ERROR: Making headers into cross compiler's win32-image install image directory"

make DESTROOT="$WORKING_DIR/arm-linux-image/pic32-tools" install-headers 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
if [ "x$SKIPLIBS" == "x" ]; then
    ...
    if [ "x$LINUX32IMAGE" != "x" ]; then
        make DESTROOT="$WORKING_DIR/$LINUX32IMAGE/pic32-tools" install -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making libraries for linux32-image cross build"
    fi

    make DESTROOT="$WORKING_DIR/export-image/pic32-tools" install -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making libraries for export-image cross build"
    make DESTROOT="$WORKING_DIR/win32-image/pic32-tools" install -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making libraries for win32-image cross build"
    make DESTROOT="$WORKING_DIR/arm-linux-image/pic32-tools" install -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
if [ "x$SKIPLINUX32" == "x" ]; then
    ...
    if [ "x$LINUX32IMAGE" != "x" ]; then
        ...
        echo "$DATE $TIME Creating linux cross build in $WORKING_DIR/linux32-build..." 2>&1 | tee -a "$LOGFILE"
        cd "$WORKING_DIR"
        if [ -e linux32-build ]; then
            rm -rf linux32-build
        fi
        mkdir linux32-build
        ...
        echo "$DATE $TIME Configuring linux32 binutils build in $WORKING_DIR/linux32-build..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/binutils/configure \
            ...
            --enable-poison-system-directories --program-prefix=pic32- --with-bugurl=https://github.com/CyberCastle/PIC32-MCU-Compiler --disable-werror 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: configuring linux32 binutils build"

        # Make linux-cross binutils and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/binutils and installing..." 2>&1 | tee -a "$LOGFILE"
        make all CFLAGS="-O2 -DMCHP_VERSION=${MCHP_VERSION}" -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing linux32 Canadian-cross binutils build"
        make CFLAGS="-O2" install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing linux32 Canadian-cross binutils build"
        ...
        echo "$DATE $TIME Configuring linux gmp build in $WORKING_DIR/linux32-build/gmp..." 2>&1 | tee -a "$LOGFILE"
        CFLAGS="-fexceptions" ../../pic32-compiler/src48x/gmp/configure \
            ...
            --enable-hash-synchronization "--with-host-libstdcxx=-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" 2>&1 | tee -a "$LOGFILE" >>/dev/null

        # Make linux gmp and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/gmp and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing gmp build"
        make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing gmp build"
        ...
        if [ "x$SKIPGRAPHITE" == "x" ]; then
            ...
            cd ppl
            echo "$DATE $TIME Configuring linux32 ppl build in $WORKING_DIR/linux32-build/ppl..." 2>&1 | tee -a "$LOGFILE"
            ../../pic32-compiler/src48x/ppl/configure \
                ...
                --with-libgmp-prefix=$WORKING_DIR/linux32-build/linux-libs --with-gmp=$WORKING_DIR/linux32-build/linux-libs 2>&1 | tee -a "$LOGFILE" >>/dev/null

            # Make native ppl and install it
            echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/ppl and installing..." 2>&1 | tee -a "$LOGFILE"
            make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
            assert_success $? "ERROR: making/installing ppl build"
            make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
            assert_success $? "ERROR: making/installing ppl build"
            ...
            echo "$DATE $TIME Configuring linux cloog build in $WORKING_DIR/linux32-build/cloog..." 2>&1 | tee -a "$LOGFILE"
            ../../pic32-compiler/src48x/cloog/configure \
                $BUILDMACHINE --enable-optimization=speed --with-gnu-ld '--with-host-libstdcxx=-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm' \
                --prefix=$WORKING_DIR/linux32-build/linux-libs --host=$LINUX32_HOST_PREFIX --with-gmp=$WORKING_DIR/linux32-build/linux-libs \
                --with-ppl=$WORKING_DIR/linux32-build/linux-libs --target=pic32mx --disable-shared --enable-static --disable-shared 2>&1 | tee -a "$LOGFILE" >>/dev/null

            # Make native cloog and install it
            echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/cloog and installing..." 2>&1 | tee -a "$LOGFILE"
            make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
            assert_success $? "ERROR: making/installing cloog build"
            make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
            assert_success $? "ERROR: making/installing cloog build"
        ...
        cd libelf
        echo "$DATE $TIME Configuring native libelf build in $WORKING_DIR/linux32-build/libelf..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/libelf/configure \
            --prefix=$WORKING_DIR/linux32-build/linux-libs --host=$LINUX32_HOST_PREFIX --target=pic32mx --disable-shared --disable-debug --disable-nls 2>&1 | tee -a "$LOGFILE" >>/dev/null

        # Make native libelf and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/libelf and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing libelf build"
        make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing libelf build"
        ...
        cd zlib
        echo "$DATE $TIME Configuring linux zlib build in $WORKING_DIR/linux32-build/zlib..." 2>&1 | tee -a "$LOGFILE"
        CC=$LINUX32_HOST_PREFIX-gcc AR="$LINUX32_HOST_PREFIX-ar" RANLIB=$LINUX32_HOST_PREFIX-ranlib \
            ./configure --prefix=$WORKING_DIR/linux32-build/linux-libs 2>&1 | tee -a "$LOGFILE" >>/dev/null

        # Make linux zlib and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/zlib and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
        ...
        echo "$DATE $TIME Configuring linux cross compiler build in $WORKING_DIR/linux32-build..." 2>&1 | tee -a "$LOGFILE"
        ...
        AR_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/pic32mx/bin/ar" \
        	...
        	-D__OPTIMIZE_SIZE__ -Os -fshort-wchar -fno-unroll-loops -fno-enforce-eh-specs -ffunction-sections -fdata-sections" --enable-cxx-flags="-fno-rtti -fno-exceptions \
        	-fomit-frame-pointer -DPREFER_SIZE_OVER_SPEED -D__OPTIMIZE_SIZE__ -Os -fshort-wchar -fno-unroll-loops -fno-enforce-eh-specs -ffunction-sections -fdata-sections" \
            $SUPPORT_HOSTED_LIBSTDCXX 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: configuring linux32 cross build"

        # Make cross compiler and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/linux32-build/gcc and installing..." 2>&1 | tee -a "$LOGFILE"
        make CFLAGS="-O2" CXXFLAGS="-O2" all-gcc \
            ...
            LD_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/pic32mx/bin/ld" \
            GCC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc" \
            CC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc" -j4 2>&1 | tee -a "$LOGFILE"
        assert_success $? "ERROR: making/installing linux Canadian-cross compiler build"

        make CFLAGS="-O2" CXXFLAGS="-O2" install-gcc 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing linux Canadian-cross compiler build"
        ...
        if [ "xSKIPMULTIPLENEWLIB" != x ]; then
            ...
            echo "$DATE $TIME Configure newlib for $LINUX32IMAGE..." 2>&1 | tee -a "$LOGFILE"
    	...
    	-D__OPTIMIZE_SIZE__ -Os -fshort-wchar -fno-unroll-loops -fno-enforce-eh-specs -ffunction-sections -fdata-sections" 2>&1 | tee -a "$LOGFILE" >>/dev/null

            echo "$DATE $TIME Make newlib for $LINUX32IMAGE..." 2>&1 | tee -a "$LOGFILE"
        ...
        echo "$DATE $TIME Configure gcc after making Newlib for $LINUX32IMAGE..." 2>&1 | tee -a "$LOGFILE"
        GCC_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc \
            ...
            XGCC_FLAGS_FOR_TARGET="-fno-rtti -fno-exceptions -fomit-frame-pointer -DPREFER_SIZE_OVER_SPEED -D__OPTIMIZE_SIZE__ -Os -fshort-wchar -fno-unroll-loops \
        	-fno-enforce-eh-specs -ffunction-sections -fdata-sections" --enable-cxx-flags="-fno-rtti -fno-exceptions -fomit-frame-pointer -DPREFER_SIZE_OVER_SPEED \
        	-D__OPTIMIZE_SIZE__ -Os -fshort-wchar -fno-unroll-loops -fno-enforce-eh-specs -ffunction-sections -fdata-sections" $SUPPORT_HOSTED_LIBSTDCXX 2>&1 | tee -a "$LOGFILE" >>/dev/null
        ...
        make CFLAGS="-O2" CXXFLAGS="-O2" all \
            ...
            AR_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-ar \
            RANLIB_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-ranlib \
            READELF_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-readelf S TRIP_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-strip \
            AS_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/pic32mx/bin/as \
            LD_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/pic32mx/bin/ld -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
        make CFLAGS="-O2" CXXFLAGS="-O2" install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        ...
        cd gdb
        echo "$DATE $TIME Configuring gdb in $WORKING_DIR/linux32-build..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/gdb/configure \
            --prefix=$WORKING_DIR/$NATIVEIMAGE/pic32-tools \
            --bindir="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/bin" \
        ...
        echo "$DATE $TIME Building gdb in $WORKING_DIR/linux32-build..." 2>&1 | tee -a "$LOGFILE"
        make 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
if [ "x$SKIPWIN32" == "x" ]; then
    ...
    echo "$DATE $TIME Creating cross build in $WORKING_DIR/win32-build..." 2>&1 | tee -a "$LOGFILE"
    if [ -e win32-build ]; then
        rm -rf win32-build
    fi
    mkdir win32-build
    ...
    echo "$DATE $TIME Configuring win32 binutils build in $WORKING_DIR/win32-build..." 2>&1 | tee -a "$LOGFILE"
    ../../pic32-compiler/src48x/binutils/configure \
        ...
        --disable-threads --disable-bootstrap --with-dwarf2 --enable-multilib --without-newlib --disable-sim --with-lib-path=: --enable-poison-system-directories \
        --program-prefix=pic32- --with-bugurl=https://github.com/CyberCastle/PIC32-MCU-Compiler --disable-werror 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: configuring win32 binutils build"

    # Make MinGW32-cross binutils and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/binutils and installing..." 2>&1 | tee -a "$LOGFILE" >>/dev/null
    make CFLAGS="-O2 -D_WIN32_WINNT=0x0501 -DWINVER=0x501 -DMCHP_VERSION=${MCHP_VERSION}" all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing win32 Canadian-cross binutils build"
    make CFLAGS="-O2 -D_WIN32_WINNT=0x0501 -DWINVER=0x501 -DMCHP_VERSION=${MCHP_VERSION}" install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing win32 Canadian-cross binutils build"
    cd ..
    ...
    echo "$DATE $TIME Configuring win32 gmp build in $WORKING_DIR/win32-build/gmp..." 2>&1 | tee -a "$LOGFILE"
    CPPFLAGS="-fexceptions" ../../pic32-compiler/src48x/gmp/configure \
        ...
        --with-gnu-ld --disable-debug --disable-rpath --enable-fft "--with-host-libstdcxx=-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" 2>&1 | tee -a "$LOGFILE" >>/dev/null

    # Make win32 gmp and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/gmp and installing..." 2>&1 | tee -a "$LOGFILE"
    make CPPFLAGS="-fexceptions" all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing gmp build"
    make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing gmp build"
    ...
    if [ "x$SKIPGRAPHITE" == "x" ]; then
        ...
        cd ppl
        echo "$DATE $TIME Configuring native ppl build in $WORKING_DIR/win32-build/ppl..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/ppl/configure \
            ...
            --enable-optimization=speed --disable-rpath --with-gmp-=$WORKING_DIR/win32-build/host-libs --with-libgmp-prefix=$WORKING_DIR/win32-build/host-libs 2>&1 | tee -a "$LOGFILE" >>/dev/null

        # Make native ppl and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/ppl and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing ppl build"
        make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing ppl build"
        ...
        echo "$DATE $TIME Configuring win32 cloog build in $WORKING_DIR/win32-build/cloog..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/cloog/configure \
        ...
        # Make native cloog and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/cloog and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j2 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing cloog build"
        make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing cloog build"
    ...
    cd libelf
    echo "$DATE $TIME Configuring native libelf build in $WORKING_DIR/win32-build/libelf..." 2>&1 | tee -a "$LOGFILE"
    GCC_FOR_TARGET='pic32-gcc' \
        ...
        CFLAGS_FOR_BUILD="-Os" \
        ../../pic32-compiler/src48x/libelf/configure \
        --prefix=$WORKING_DIR/win32-build/host-libs --host=$MINGW32_HOST_PREFIX $BUILDMACHINE --target=pic32mx --disable-shared --disable-debug --disable-nls 2>&1 | tee -a "$LOGFILE" >>/dev/null

    # Make native libelf and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/libelf and installing..." 2>&1 | tee -a "$LOGFILE"
    make all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing libelf build"
    make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing libelf build"
    ...
    cd zlib
    echo "$DATE $TIME Configuring win32 zlib build in $WORKING_DIR/win32-build/zlib..." 2>&1 | tee -a "$LOGFILE"
    CC=$MINGW32_HOST_PREFIX-gcc AR="$MINGW32_HOST_PREFIX-ar" RANLIB=$MINGW32_HOST_PREFIX-ranlib ./configure --prefix=$WORKING_DIR/win32-build/host-libs 2>&1 | tee -a "$LOGFILE" >>/dev/null

    # Make win32 zlib and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/zlib and installing..." 2>&1 | tee -a "$LOGFILE"
    make all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing zlib build - all"
    make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing zlib build - install"
    ...
    echo "$DATE $TIME Configuring win32 cross compiler build in $WORKING_DIR/win32-build..." 2>&1 | tee -a "$LOGFILE"
    ...
    if [ "x$NATIVEIMAGE" == "xLinux-image" ]; then
        CC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc" \
			...
			-fno-enforce-eh-specs -ffunction-sections -fdata-sections" --enable-cxx-flags="-fno-rtti -fno-exceptions -fomit-frame-pointer -DPREFER_SIZE_OVER_SPEED \
			-D__OPTIMIZE_SIZE__ -Os -fshort-wchar -fno-unroll-loops -fno-enforce-eh-specs -ffunction-sections -fdata-sections" \
            $SUPPORT_HOSTED_LIBSTDCXX "--with-host-libstdcxx=-static-libstdc++ -Wl,-Bstatic,-lstdc++,-lwinpthread,-Bdynamic,-lm" 2>&1 | tee -a "$LOGFILE" >>/dev/null
    else
    ...
    # Make cross compiler and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/win32-build/gcc and installing..." 2>&1 | tee -a "$LOGFILE"
    make CFLAGS="-Os -D_WIN32_WINNT=0x0501 -DWINVER=0x501" CXXFLAGS="-Os -D_WIN32_WINNT=0x0501 -DWINVER=0x501" all-gcc \
        ...
        LD_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/pic32mx/bin/ld" \
        GCC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc" \
        CC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc" -j4 CXXFLAGS="$CXXFLAGS_FOR_TARGET -Os" 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing win32 Canadian-cross compiler build"

    make CFLAGS="-Os -D_WIN32_WINNT=0x0501 -DWINVER=0x501" install-gcc 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing win32 Canadian-cross compiler build"
    ...
    if [ "xSKIPMULTIPLENEWLIB" != x ]; then
        ...
        echo "$DATE $TIME Configure newlib for win32-image..." 2>&1 | tee -a "$LOGFILE"
        ...
        echo "$DATE $TIME Make newlib for win32-image..." 2>&1 | tee -a "$LOGFILE"
    ...
    echo "$DATE $TIME Configure gcc after making Newlib for win32-image..." 2>&1 | tee -a "$LOGFILE"
    GCC_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc \
        CC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc -I$WORKING_DIR/pic32-compiler/src48x/gcc/gcc/ginclude" \
        CXX_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-g++ \
        CPP_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-g++ \
		...
		-fno-enforce-eh-specs -ffunction-sections -fdata-sections" $SUPPORT_HOSTED_LIBSTDCXX 2>&1 | tee -a "$LOGFILE" >>/dev/null
    ...
    make CFLAGS="-O2 -D_WIN32_WINNT=0x0501 -DWINVER=0x501" CXXFLAGS="-O2 -D_WIN32_WINNT=0x0501 -DWINVER=0x501" install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    ...
    cd gdb
    echo "$DATE $TIME Configuring gdb in $WORKING_DIR/win32-build..." 2>&1 | tee -a "$LOGFILE"
    ../../pic32-compiler/src48x/gdb/configure \
        ...
        --disable-gdbtk 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: Configure gdb for native build"
    echo "$DATE $TIME Building gdb in $WORKING_DIR/win32-build..." 2>&1 | tee -a "$LOGFILE"
    make 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
if [ "x$SKIPARMLINUX" == "x" ]; then
    ...
    echo "$DATE $TIME Creating cross build in $WORKING_DIR/arm-linux-build..." 2>&1 | tee -a "$LOGFILE"
    if [ -e arm-linux-build ]; then
    ...
    echo "$DATE $TIME Configuring arm-linux binutils build in $WORKING_DIR/arm-linux-build..." 2>&1 | tee -a "$LOGFILE"
    ../../pic32-compiler/src48x/binutils/configure \
    ...
    echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/binutils and installing..." 2>&1 | tee -a "$LOGFILE"
    make CFLAGS="-Os -DMCHP_VERSION=${MCHP_VERSION}" all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing arm-linux Canadian-cross binutils build"

    make CFLAGS="-Os -DMCHP_VERSION=${MCHP_VERSION}" install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing arm-linux Canadian-cross binutils build"
    cd ..
    ...
    echo "$DATE $TIME Configuring arm-linux gmp build in $WORKING_DIR/arm-linux-build/gmp..." 2>&1 | tee -a "$LOGFILE"
    CPPFLAGS="-fexceptions" ../../pic32-compiler/src48x/gmp/configure \
        ...
        --disable-rpath --enable-fft "--with-host-libstdcxx=-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" 2>&1 | tee -a "$LOGFILE" >>/dev/null

    # Make arm-linux gmp and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/gmp and installing..." 2>&1 | tee -a "$LOGFILE"
    make CPPFLAGS="-fexceptions" all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing gmp build"
    ...
    if [ "x$SKIPGRAPHITE" == "x" ]; then
        ...
        cd ppl
        echo "$DATE $TIME Configuring native ppl build in $WORKING_DIR/arm-linux-build/ppl..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/ppl/configure \
            ...
            --with-libgmp-prefix=$WORKING_DIR/arm-linux-build/host-libs 2>&1 | tee -a "$LOGFILE" >>/dev/null

        # Make native ppl and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/ppl and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing ppl build"
        make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing ppl build"
        ...
        echo "$DATE $TIME Configuring arm-linux cloog build in $WORKING_DIR/arm-linux-build/cloog..." 2>&1 | tee -a "$LOGFILE"
        ../../pic32-compiler/src48x/cloog/configure \
            ...
            --enable-static --disable-shared 2>&1 | tee -a "$LOGFILE" >>/dev/null

        # Make native cloog and install it
        echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/cloog and installing..." 2>&1 | tee -a "$LOGFILE"
        make all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing cloog build"
        make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
        assert_success $? "ERROR: making/installing cloog build"
    ...
    cd libelf
    echo "$DATE $TIME Configuring native libelf build in $WORKING_DIR/arm-linux-build/libelf..." 2>&1 | tee -a "$LOGFILE"
    GCC_FOR_TARGET='pic32-gcc' \
        CC_FOR_TARGET='pic32-gcc' \
        CPP_FOR_TARGET='pic32-g++' \
        AS_FOR_TARGET=pic32-as \
        ...
        --host=$ARMLINUX32_HOST_PREFIX $BUILDMACHINE --target=pic32mx --disable-shared --disable-debug --disable-nls 2>&1 | tee -a "$LOGFILE" >>/dev/null

    # Make native libelf and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/libelf and installing..." 2>&1 | tee -a "$LOGFILE"
    make all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing libelf build"
    make install 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing libelf build"
    ...
    cd zlib
    echo "$DATE $TIME Configuring arm-linux zlib build in $WORKING_DIR/arm-linux-build/zlib..." 2>&1 | tee -a "$LOGFILE"
    CC=$ARMLINUX32_HOST_PREFIX-gcc AR="$ARMLINUX32_HOST_PREFIX-ar" RANLIB=$ARMLINUX32_HOST_PREFIX-ranlib \
        ./configure --prefix=$WORKING_DIR/arm-linux-build/host-libs 2>&1 | tee -a "$LOGFILE" >>/dev/null

    # Make arm-linux zlib and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/zlib and installing..." 2>&1 | tee -a "$LOGFILE"
    make all -j4 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing zlib build - all"
    ...
    echo "$DATE $TIME Configuring arm-linux cross compiler build in $WORKING_DIR/arm-linux-build..." 2>&1 | tee -a "$LOGFILE"
    ...
    # Make cross compiler and install it
    echo "$DATE $TIME Making all in $WORKING_DIR/arm-linux-build/gcc and installing..." 2>&1 | tee -a "$LOGFILE"
    ...
    make CFLAGS="-Os" install-gcc 2>&1 | tee -a "$LOGFILE" >>/dev/null
    assert_success $? "ERROR: making/installing arm-linux Canadian-cross compiler build"
    ...
    if [ "xSKIPMULTIPLENEWLIB" != x ]; then
        ...
        #build newlib here
        echo "$DATE $TIME Configure newlib for arm-linux-image..." 2>&1 | tee -a "$LOGFILE"
        GCC_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/bin/pic32-gcc \
            CC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/bin/pic32-gcc -I$WORKING_DIR/pic32-compiler/src48x/gcc/gcc/ginclude" \
            CXX_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/bin/pic32-g++ \
            CPP_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/bin/pic32-g++ \
        ...
        echo "$DATE $TIME Make newlib for arm-linux-image..." 2>&1 | tee -a "$LOGFILE"
        make all -j4 CFLAGS_FOR_TARGET="-fno-rtti -fno-exceptions -fomit-frame-pointer -DPREFER_SIZE_OVER_SPEED -D__OPTIMIZE_SIZE__ -Os -fshort-wchar \
		-fno-unroll-loops -fno-enforce-eh-specs -ffunction-sections -fdata-sections -DSMALL_MEMORY -D_NO_GETLOGIN -D_NO_GETPWENT -D_NO_GETUT -D_NO_GETPASS -D_NO_SIGSET" \
    ...
    echo "$DATE $TIME Configure gcc after making Newlib for arm-linux-image..." 2>&1 | tee -a "$LOGFILE"
    GCC_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc \
        CC_FOR_TARGET="$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-gcc -I$WORKING_DIR/pic32-compiler/src48x/gcc/gcc/ginclude" \
        CXX_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-g++ \
        CPP_FOR_TARGET=$WORKING_DIR/$NATIVEIMAGE/pic32-tools/bin/pic32-g++ \
    ...
    make CFLAGS="-Os" CXXFLAGS="-Os" all \
        ...
        CXXFLAGS="$CXXFLAGS_FOR_TARGET -Os" 2>&1 | tee -a "$LOGFILE" >>/dev/null

    make CFLAGS="-Os" CXXFLAGS="-Os" install 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
###
echo "$DATE $TIME Removing unecessary directories from build..." 2>&1 | tee -a "$LOGFILE"
...
echo "Making zip files" | tee -a "$LOGFILE"
#ZIP installation directory.

echo "$DATE $TIME Tar components to $WORKING_DIR/zips directory..." | tee -a "$LOGFILE"
echo "$DATE $TIME Tar installation directory..." | tee -a "$LOGFILE"
cd "$WORKING_DIR"
if [[ ! -e zips ]]; then
    mkdir zips
fi
...
tar -czvf $WORKING_DIR/zips/pic32-tools-$REV-win32-image.tar.gz pic32-tools 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
find . -name *_pic.o -delete
find . -name *-pic.o -delete
tar -czvf $WORKING_DIR/zips/pic32-tools-$REV-export-image.tar.gz pic32-tools 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
find . -name *_pic.o -delete
find . -name *-pic.o -delete
tar -czvf $WORKING_DIR/zips/pic32-tools-$REV-arm-linux-image.tar.gz pic32-tools 2>&1 | tee -a "$LOGFILE" >>/dev/null
...
if [ "x$LINUX32IMAGE" != "x" ]; then
    ...
    find . -name *_pic.o -delete
    find . -name *-pic.o -delete
    tar -czvf $WORKING_DIR/zips/pic32-tools-$REV-$LINUX32IMAGE.tar.gz pic32-tools 2>&1 | tee -a "$LOGFILE" >>/dev/null
    cd ..
fi
